...
void Bank::sellPropertyToPlayerByID(int Propertyid, Player& player) {
	...
	player.deductCash(List[Propertyid][0].getPrice());
	List[Propertyid][0].setOwnerID(player.getID());
}
...
void Bank::purchasePropertyFromPlayerByID(int Propertyid, Player& player) {
	...
	player.addCash(List[Propertyid][0].getPrice());
...
void Bank::payMortageToPlayerByID(int Propertyid, Player& player) {
	int mortage = 0;
	mortage = List[Propertyid][0].getPrice() / 2.0;
...
void Bank::declarePlayerBankruptByID(Player player) {
	...
	if (player.getCash() == 0) {
		for (int i = 0; i < size; i++) {
			if (List[i][0].getOwnerID() == player.getID()) {
				found = true;
			}
		}
		if (found == false) {
