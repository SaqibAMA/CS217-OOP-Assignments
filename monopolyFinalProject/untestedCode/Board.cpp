#include "Board.h"
#include <fstream>
//UPDATED ACCORDING TO NEW DATA MEMBERS NEW FUNCITONS ADDED AT THE END
//GETTER SETTER OF NEW DATA MEMBERS ADDED
Board::Board() {
	ifstream CC;
	ifstream CCC;
	CC.open("Chance.txt");
	CCC.open("CommunityChest.txt");
	prevTurns = nullptr;
	playerCount = 0;
	...
	for (int i = 0; i < 20; i++)
		cells[privatePropertySpaces[i]] = new Space;
	...
	for (int i = 0; i < 15; i++) {
		char CCtemp[200];
		char CCCtemp[200];
		CC.getline(CCtemp, 200);
		CCC.getline(CCCtemp, 200);
